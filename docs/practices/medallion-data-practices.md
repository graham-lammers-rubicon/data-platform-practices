...
Silver tables declare their grain in the COMMENT; that is the gate. Tidy shape (long and atomic: one variable per column, one observation per row, wide sources unpivoted, compound codes split into typed columns) is an optional, low-priority practice: the Silver-long / Gold-wide reshaping is a hard transition for people and tooling, so adopt it per domain when capacity allows and do not block delivery on it. A Silver table closer to the source shape is acceptable when the grain is declared, columns are typed, and additivity is labeled. Where you do unpivot, one sharp edge: a fixed unpivot over a volatile source silently drops new columns (they land in `_rescued_data` and go no further); monitor `_rescued_data` for non-empty rows and restructure when the shape stabilizes ([Tidy data](tidy-data.md): an analysis standard, not a storage mandate).
