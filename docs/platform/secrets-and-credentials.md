...
- Vaults backing scopes use the **Vault access policy** permission model. Azure RBAC is not supported for Key Vault-backed scopes.
...
- CI holds no Databricks credentials: deploy auth is OIDC federation, which Databricks documents as "the most secure way to authenticate" for CI/CD. GitHub Actions secrets store nothing that Key Vault or federation can carry; they are CI plumbing, not a secret store, invisible to Databricks workloads and without expiration or rotation.
...
- Terraform never places secret values in configuration; state stores them in plain text. Terraform creates the vault, access policies, and scope; values are set out-of-band. State is remote, encrypted, and access-controlled per HashiCorp guidance: "treat your state file as sensitive data."
...
- Redaction replaces only literal values with `[REDACTED]`. Any transformation of the secret prints plainly; the docs are explicit that redaction "does not prevent deliberate and arbitrary transformations." ACLs are the control, not redaction.
- Workspace admins, scope creators, and granted users can read secret contents; Databricks states it "is not possible to fully prevent these users from viewing secret contents." Grant accordingly.
