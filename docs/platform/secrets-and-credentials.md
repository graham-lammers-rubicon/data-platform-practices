...
- One vault per scope, 1:1. A scope grants access to every secret in its backing vault; the vault is the access boundary. Scope `<domain>-<env>` is backed by vault `kv-<domain>-<env>-[region]-001` ([Naming conventions](naming-conventions.md)).
