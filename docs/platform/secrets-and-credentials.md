...
1. **No secret.** Workload identity federation for CI ([CI/CD](github-cicd-and-deployment.md)), Unity Catalog storage and service credentials backed by access connector managed identities for Azure services. Databricks documents managed identities as ["strongly recommended"](https://learn.microsoft.com/en-us/azure/databricks/connect/unity-catalog/cloud-services/service-credentials) over service principals: they work behind network rules and "remove the need to manage and rotate secrets."
2. **Azure Key Vault-backed secret scope.** The only storage for secrets that must exist: third-party system credentials (`salesforce-client-secret`), OAuth M2M client secrets ([Service principal authentication](databricks-service-principal-auth.md)). The scope is a read-only interface; the secret lives in the vault.
...
- One vault per scope, 1:1. A scope grants access to every secret in its backing vault; the vault is the access boundary. Scope `<domain>-<env>` is backed by vault `[subject]-<domain>-vault-[region]-<env>` ([Naming conventions](naming-conventions.md)).
- Vaults backing scopes use the **Vault access policy** permission model. Per the [Databricks secrets doc](https://learn.microsoft.com/en-us/azure/databricks/security/secrets/) (re-verified 2026-07): "Azure Key Vault-backed secret scopes only support the Vault access policy permission model. Azure role-based access control (RBAC) is not supported." Microsoft's general Key Vault guidance prefers RBAC; this is a documented Databricks platform constraint, not a choice. Re-check at each review; if RBAC support ships, or Unity Catalog secrets reach GA (watch item above), revisit this rule.
- Vaults, scopes, and scope ACLs are Terraform-created, per the everything-as-code rule ([Environments](databricks-environments.md)). Scope ACLs grant READ to the consuming pipeline SP or group only; MANAGE stays with the platform team.
...
- CI holds no Databricks credentials: deploy auth is OIDC federation, which Databricks documents as ["the most secure way to authenticate"](https://learn.microsoft.com/en-us/azure/databricks/dev-tools/bundles/ci-cd-bundles) for CI/CD. GitHub Actions secrets store nothing that Key Vault or federation can carry; they are CI plumbing, not a secret store.
...
- The 24-character Key Vault name budget breaks for long subject or domain tokens (a four-character subject with `marketing` as domain, `vault` as type, a four-character region, and `np` already resolves to 28). Check the budget per name; abbreviate the environment to `np` only where forced, and register any shortened token in the naming conventions before first use.
...
- GitHub-hosted runners run on GitHub's infrastructure. A vault locked to private endpoints or a strict firewall is unreachable from them; the runtime-fetch pattern needs the vault firewall to admit the runner, or self-hosted runners with network access. Verify reachability before making a workflow depend on it.
...
- Databricks: [CI/CD on Databricks](https://learn.microsoft.com/en-us/azure/databricks/dev-tools/bundles/ci-cd-bundles) (federation "eliminates the need for Databricks secrets")
